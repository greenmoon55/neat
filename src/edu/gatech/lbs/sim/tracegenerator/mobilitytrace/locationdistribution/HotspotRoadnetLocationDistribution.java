...
public class HotspotRoadnetLocationDistribution implements ILocationDistribution {
  ...
  public HotspotRoadnetLocationDistribution(RoadMap roadmap, int hotspotCount, double coeff, long seed, int destNum) {
  ...
  //generate destinations - binh
  public IVector[] getFixedLocations() {
	  IVector[] poi = new IVector[destNum];
	  int segNum =  (int) Math.floor(segments.length/destNum);
	  for (int k=0; k<destNum; k++){
		  
		  RoadSegment seg = segments[(k+1)*segNum-1];
		  poi[k] = new RoadnetVector(seg,(float) (rnd.nextDouble() * seg.getLength()));
	  }
