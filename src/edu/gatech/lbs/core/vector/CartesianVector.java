...
public class CartesianVector implements IVector {
  ...
  public void saveToTxt(PrintWriter out) throws IOException{
	  ...
	  //out.print("f ");
	  out.print(Double.toString(x)+ " ");
	  out.print(Double.toString(y)+ " ");
  }
  public double toPoint(CartesianVector p){
		double distance;
		distance = (double)Math.sqrt(this.distanceSquared(p));   
		return distance;
		
	}

	public double distanceSquared(CartesianVector p){
		double distance;
		double dx;
		double dy;
		dx = p.getX()-this.getX();
